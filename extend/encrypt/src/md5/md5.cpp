...
void MD5::md5_process(MD5_CTX *ctx, const unsigned char data[64])
{
...
string MD5::md5file(const string& fileFullName)
{
...
void MD5::md5update(MD5_CTX *ctx, unsigned char *input, unsigned int ilen)
{
    unsigned int fill;
    unsigned long left;
    if (ilen <= 0)
...
void MD5::md5final(unsigned char output[16], MD5_CTX *ctx)
{
...
string MD5::md5bin(const string &sString)
{
    unsigned char sOutBuffer[16];
...
string MD5::md5str(const string &sString)
{
    string s = md5bin(sString);
    return bin2str((const void *)s.data(), s.length(), "");
}

string MD5::bin2str(const void *buf, size_t len, const string &sSep)
{
    if (buf == NULL || len <= 0)
    {
        return "";
