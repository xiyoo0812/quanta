...
-- 连接关闭回调
function OnlineAgent:on_socket_error(client, err)
...
-- 连接成回调
function OnlineAgent:on_socket_connect(client)
    log_info("[OnlineAgent][on_socket_connect]: connect monitor success!")
    -- 到monitor注册
    self.client:send("rpc_monitor_register", quanta.id, quanta.service_id, quanta.index, quanta.name)
end

-- 请求服务
function OnlineAgent:service_request(api_name, data)
    local req = {
        data = data,
        id  = quanta.id,
        index = quanta.index,
...
-- 处理Monitor通知退出消息
function OnlineAgent:on_quanta_quit(reason)
    -- 发个退出通知
    ...
    timer_mgr:once(PeriodTime.SECOND_MS, function()
        log_warn("[OnlineAgent][on_quanta_quit]->service:%s", quanta.name)
        signal_quit()
    end)
    return { code = 0 }
end
...
--执行远程rpc消息
function OnlineAgent:on_remote_message(data, message)
    if not message then
    ...
    if not ok or check_failed(code) then
        log_err("[OnlineAgent][on_remote_message] web_rpc faild: ok=%s, ec=%s", ok, code)
        return { code = ok and code or KernCode.RPC_FAILED, msg = ok and "" or code}
    end
    return { code = 0 , data = res}
end

quanta.monitor = OnlineAgent()
