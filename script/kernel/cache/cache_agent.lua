...
function CacheAgent:update(primary_key, table_name, table_data, cache_name, flush)
    ...
    local ok, code = router_mgr:call_cachesvr_target(primary_key, "rpc_cache_update", quanta.id, req_data)
...
function CacheAgent:update_key(primary_key, table_name, table_key, table_value, cache_name, flush)
    ...
    local ok, code = router_mgr:call_cachesvr_target(primary_key, "rpc_cache_update_key", quanta.id, req_data)
    if not ok or check_failed(code) then
...
function CacheAgent:delete(primary_key, cache_name)
    ...
    local ok, code = router_mgr:call_cachesvr_target(primary_key, "rpc_cache_delete", quanta.id, req_data)
    if not ok or check_failed(code) then
...
function CacheAgent:flush(primary_key, cache_name)
    local req_data = { cache_name or "player", primary_key }
    local ok, code = router_mgr:call_cachesvr_target(primary_key, "rpc_cache_flush", quanta.id, req_data)
    if not ok or check_failed(code) then
