...
--协议接收
function Loki:on_recv_tcp_message(message)
    local cmd_id, cmd_name = protobuf_mgr:msg_id(message.cmd_id)
    local cmdinfo = lformat("[{:^4}][{:<40}] => {}", cmd_id, cmd_name, message.request)
    self:collect_pb(cmdinfo, "recv")
end

--协议发送
function Loki:on_send_tcp_message(cmd_id, body, send_len)
    local cmdid, cmd_name = protobuf_mgr:msg_id(cmd_id)
    local cmdinfo = lformat("[{:^4}][{:<40}] => {}", cmdid, cmd_name, body)
