...
function MongoMQ:setup(coll_name)
    ...
    local ok, code = mongo_proxy:create_indexes(query)
...
function MongoMQ:len_message(target_id)
    ...
    local ok, code, result = mongo_proxy:count(query, target_id)
    if qsuccess(code, ok) then
        return result
    end
    return 0
...
function MongoMQ:list_message(target_id)
    ...
    local ok, code, result = mongo_proxy:find(query, target_id)
    if qsuccess(code, ok) then
        return result
    end
end
...
function MongoMQ:delete_message(target_id, events)
    for _, event in ipairs(events) do
        ...
        mongo_proxy:delete({ self.coll_name, { uuid = event.uuid } }, target_id)
    end
end
...
function MongoMQ:send_message(target_id, event, args, ttl)
    ...
    local ok = mongo_proxy:insert({ self.coll_name, doc }, target_id)
    if not ok then
