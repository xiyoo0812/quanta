...
function NodeReq:on_start()
    if self.cmd_id then
        ...
        if not values then
            log_warn("[NodeReq][on_start] robot:{} run node:{}'s {} collect inputs {} failed!", role.open_id, self.name, self.cmd_id, self.inputs)
            self:failed(err)
            return
        end
        log_debug("[NodeReq][on_start] robot:{} run node:{}'s call {}=>{}", role.open_id, self.name, self.cmd_id, values)
        local ok, res = role:call(self.cmd_id, values)
        if role:check_callback(ok, res) then
            log_warn("[NodeReq][on_start] robot:{} run node:{}'s {} failed: ok={}, res={}", role.open_id, self.name, self.cmd_id, ok, res)
            event_mgr:notify_trigger("on_error_message", self.cmd_id, role.open_id, res)
            self:failed(res)
            return
        end
        log_debug("[NodeReq][on_start] robot:{} run node:{}'s {}=>{} success", role.open_id, self.name, self.cmd_id, res)
