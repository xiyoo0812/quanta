...
function LoginModule:guest_login_req()
    ...
    if self:check_callback(ok, res) then
        log_warn("[LoginModule][guest_login_req] robot:{}, ok={}, res={}", self:get_title(), ok, res)
        return false
    end
    self.roles = res.roles
    self.user_id = res.user_id
    log_debug("[LoginModule][guest_login_req] robot:{} success", self:get_title())
    return true
end
...
function LoginModule:account_login_req()
    ...
    local ok, res = self:call("NID_LOGIN_ACCOUNT_LOGIN_REQ", req_data)
    log_warn("[LoginModule][account_login_req] robot:{}, ok={}, res={}", self:get_title(), ok, req_data)
    if self:check_callback(ok, res) then
        log_warn("[LoginModule][account_login_req] robot:{}, ok={}, res={}", self:get_title(), ok, res)
        return false
    end
    self.roles = res.roles
    self.user_id = res.user_id
    log_debug("[LoginModule][account_login_req] robot:{} success", self:get_title())
    return true
end
...
function LoginModule:random_name_req()
    ...
    if self:check_callback(ok, res) then
        log_warn("[LoginModule][random_name_req] robot:{}, ok={}, res={}", self:get_title(), ok, res)
        return
    end
    log_debug("[LoginModule][random_name_req] robot:{} success", self:get_title())
    return res.name
end
...
function LoginModule:choose_role_req()
    ...
    if not role then
        log_warn("[LoginModule][choose_role_req] robot:{} roles is empty", self:get_title())
        return false
    end
    local req_data = {
    ...
    log_debug("[LoginModule][choose_role_req] robot:{} success", self:get_title())
    return true
end
...
function LoginModule:delete_role_req()
    local role = trandarray(self.roles)
    if not role then
        log_warn("[LoginModule][delete_role_req] robot:{} roles is empty", self:get_title())
        return false
    end
    local req_data = {
        role_id = role.role_id,
    ...
    if self:check_callback(ok, res) then
        log_warn("[LoginModule][delete_role_req] robot:{}, ok={}, res={}", self:get_title(), ok, res)
        return false
    end
    tdelete(self.roles, role)
    log_debug("[LoginModule][delete_role_req] robot:{} success", self:get_title())
    return true
end
...
function LoginModule:account_reload_req()
    ...
    if self:check_callback(ok, res) then
        log_warn("[LoginModule][account_reload_req] robot:{}, ok={}, res={}", self:get_title(), ok, res)
        return false
    end
    self.roles = res.roles
    self.user_id = res.user_id
    log_debug("[LoginModule][account_reload_req] robot:{} success", self:get_title())
    return true
end
...
function LoginModule:role_login_req()
    ...
    if self:check_callback(ok, res) then
        log_warn("[LoginModule][role_login_req] robot:{}, ok={}, res={}", self:get_title(), ok, res)
        return false
    end
    self.lobby_token = res.token
    log_debug("[LoginModule][role_login_req] robot:{} success", self:get_title())
    return true
end
...
function LoginModule:role_logout_req()
    ...
    if self:check_callback(ok, res) then
        log_warn("[LoginModule][role_logout_req] robot:{}, ok={}, res={}", self:get_title(), ok, res)
        return false
    end
    self.player_id = nil
    log_debug("[LoginModule][role_logout_req] robot:{} success", self:get_title())
    return true
end
...
function LoginModule:role_reload_req()
    ...
    if self:check_callback(ok, res) then
        log_warn("[LoginModule][role_reload_req] robot:{}, ok={}, res={}", self:get_title(), ok, res)
        return false
    end
    self.lobby_token = res.token
    log_debug("[LoginModule][role_reload_req] robot:{} success", self:get_title())
    return true
end
