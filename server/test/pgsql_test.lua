...
timer_mgr:once(2000, function()
    ...
    code, res_oe = pgsql_mgr:query(1, "update test_pgsql set pid = 123454, value = 20 where pid in (select pid from test_pgsql where pid = 123454 limit 1)")
    log_debug("db update code: {}, res_oe = {}", code, res_oe)
    ...
    log_debug("db delete code: {}, res_oe = {}", code, res_oe)
    code, res_oe = pgsql_mgr:query(1, "insert into test_pgsql (id, pid, value) values (1, 123457, 40)\
            on conflict (id) do update set pid = excluded.pid, value = excluded.value")
