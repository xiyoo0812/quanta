...
timer_mgr:once(2000, function()
    ...
    icode, ierr = mongo_mgr:insert(MAIN_DBID, "test_mongo_1", {pid = 123457, data = {a =1, b=2}})
    log_debug("db insert code: %s, err = %s", icode, ierr)
    local fcode, res = mongo_mgr:find(MAIN_DBID, "test_mongo_1", {}, {_id = 0})
    log_debug("db find code: %s, res = %s", fcode, res)
    local f1code, f1res = mongo_mgr:find_one(MAIN_DBID, "test_mongo_1", {pid = 123456}, {_id = 0})
    log_debug("db find code: %s, res = %s", f1code, f1res)
    local ucode, uerr = mongo_mgr:update(MAIN_DBID, "test_mongo_1", {pid = 123458, data = {a =1, b=4}}, {pid = 123457})
    ...
    icode, ierr = mongo_mgr:execute(MAIN_DBID, "listIndexes", "test_mongo_2")
