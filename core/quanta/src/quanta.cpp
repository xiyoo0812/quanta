...
cpchar quanta_app::get_env(cpchar key) {
    auto it = m_environs.find(key);
    if (it != m_environs.end()) return it->second.c_str();
    return nullptr;
}

void quanta_app::set_env(cpchar key, cpchar value, int over) {
    if (over == 1 || !m_environs.contains(key)) {
        m_environs[key] = value;
    }
}

void quanta_app::add_path(cpchar field, cpchar path) {
...
bool quanta_app::setup(int argc, cpchar argv[], lua_State* L) {
    if (L) {
...
bool quanta_app::load(int argc, cpchar argv[]) {
    //将启动参数转负责覆盖环境变量
    for (int i = 1; i < argc; ++i) {
        sstring argvi = argv[i];
        if (auto pos = argvi.find("="); pos != sstring::npos) {
        ...
        if (i == 1){
            ...
            m_lua->set_function("set_path", [&](cpchar field, cpchar path) { m_lua->set_path(field, path); set_env(field, path, 1); });
            if (!m_lua->run_script(std::format("dofile('{}')", argv[1]), [&](vstring err) {
...
bool quanta_app::init() {
    ...
    if (sandbox) {
        if (!m_lua->run_script(std::format("require '{}'", sandbox), [&](vstring err) {
    ...
    if (!m_lua->run_script(std::format("require '{}'", entry), [&](vstring err) {
