...
int socket_mgr_impl::listen(std::string& err, const char ip[], int port, eproto_type proto_type)
{
    int ret = false;
    socket_t fd = INVALID_SOCKET;
...
int socket_mgr_impl::connect(std::string& err, const char node_name[], const char service_name[], int timeout, eproto_type proto_type)
{
    if (is_full())
    {
...
void socket_mgr_impl::set_accept_callback(uint32_t token, const std::function<void(uint32_t, eproto_type eproto_type)>& cb)
{
    auto node = get_object(token);
    if (node)
    {
...
int socket_mgr_impl::accept_stream(socket_t fd, const char ip[], const std::function<void(int, eproto_type)>& cb, eproto_type proto_type)
{
